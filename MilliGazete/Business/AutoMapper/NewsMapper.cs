    ...
    public class NewsMapper : Profile
    {
        public NewsMapper()
        {
            ...
            CreateMap<News, WideHeadingNewsDto>()
                ...
                u => u.MapFrom(g =>
                g.NewsFiles.Where(t => t.NewsFileTypeEntityId == (int)NewsFileTypeEntities.MainHeadingImage)
            ...
            CreateMap<News, LiveNarrotationNewsDto>()
                .ForMember(f => f.NewsId, u => u.MapFrom(g => g.Id))
                .ForMember(f => f.ImageUrl, u => u.MapFrom(g =>
            ...
            CreateMap<News, MainPageNewsDto>()
                .ForMember(f => f.NewsId, u => u.MapFrom(g => g.Id))
                .ForMember(f => f.StyleCode, u => u.MapFrom(g => g.NewsCategories.Any() ? g.NewsCategories.First().Category.StyleCode : ""))
                .ForMember(f => f.CategoryName, u => u.MapFrom(g => g.NewsCategories.Any() ? g.NewsCategories.First().Category.CategoryName : ""))
                .ForMember(f => f.Url, u => u.MapFrom(g => g.Url.GetUrl(g.HistoryNo, g.NewsTypeEntityId, g.NewsCategories.Select(e => e.Category.Url).FirstOrDefault())))
            ...
            CreateMap<News, MainPageVideoNewsDto>()
                .ForMember(f => f.NewsId, u => u.MapFrom(g => g.Id))
                .ForMember(f => f.Url, u => u.MapFrom(g => g.Url.GetUrl(g.HistoryNo, g.NewsTypeEntityId, g.NewsCategories.Select(e => e.Category.Url).FirstOrDefault())))
                .ForMember(f => f.VideoUrl, u => u.MapFrom(g =>
                g.NewsFiles.Where(t => t.NewsFileTypeEntityId == (int)NewsFileTypeEntities.GalleryVideo)
                .Select(w => w.File.FileName).FirstOrDefault().GetFullFilePath()))
                .ForMember(f => f.VideoCoverImageUrl, u => u.MapFrom(g =>
                g.NewsFiles.Where(t => t.NewsFileTypeEntityId == (int)NewsFileTypeEntities.GalleryVideo)
                .Select(w => w.VideoCoverFile.FileName).FirstOrDefault().GetFullFilePath()));

            CreateMap<News, MainPageImageNewsDto>()
                .ForMember(f => f.NewsId, u => u.MapFrom(g => g.Id))
                .ForMember(f => f.Url, u => u.MapFrom(g => g.Url.GetUrl(g.HistoryNo, g.NewsTypeEntityId, g.NewsCategories.Select(e => e.Category.Url).FirstOrDefault())))
                .ForMember(f => f.ImageUrl, u => u.MapFrom(g =>
                g.NewsFiles.Where(t => t.NewsFileTypeEntityId == (int)NewsFileTypeEntities.GalleryPhoto)
                .Select(w => w.File.FileName).FirstOrDefault().GetFullFilePath()));
          ...
          .ForMember(f => f.NewsAgencyEntity, u => u.MapFrom(g => g.NewsAgencyEntity == null ? "" : Translator.GetByKey(g.NewsAgencyEntity.EntityName)))
          .ForMember(f => f.NewsTypeEntity, u => u.MapFrom(g => g.NewsTypeEntity == null ? "" : Translator.GetByKey(g.NewsTypeEntity.EntityName)))
          .ForMember(f => f.Thumbnail, u => u.MapFrom(g =>
                 g.NewsFiles.Where(t => t.NewsFileTypeEntityId == (int)NewsFileTypeEntities.NormalImage)
                 .Select(w => w.File.FileName).FirstOrDefault().GetFullFilePath()))
